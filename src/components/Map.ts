...
export class MapComponent {
  ...
  private createLayerToggles(): HTMLElement {
    ...
    layers.forEach((layer) => {
      ...
      btn.textContent = t(layerLabels[layer] || layer);
  ...
  private showLayerHelp(): void {
    ...
    const techHelpContent = `
      ...
      <div class="layer-help-content">
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Tech Ecosystem')}</div>
          <div class="layer-help-item"><span>${t('STARTUPHUBS')}</span> ${t('Major startup ecosystems (SF, NYC, London, etc.)')}</div>
          <div class="layer-help-item"><span>${t('CLOUDREGIONS')}</span> ${t('AWS, Azure, GCP data center regions')}</div>
          <div class="layer-help-item"><span>${t('TECHHQS')}</span> ${t('Headquarters of major tech companies')}</div>
          <div class="layer-help-item"><span>${t('ACCELERATORS')}</span> ${t('Y Combinator, Techstars, 500 Startups locations')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Infrastructure')}</div>
          <div class="layer-help-item"><span>${t('CABLES')}</span> ${t('Major undersea fiber optic cables (internet backbone)')}</div>
          <div class="layer-help-item"><span>${t('DATACENTERS')}</span> ${t('AI compute clusters ≥10,000 GPUs')}</div>
          <div class="layer-help-item"><span>${t('OUTAGES')}</span> ${t('Internet blackouts and service disruptions')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Natural & Economic')}</div>
          <div class="layer-help-item"><span>${t('NATURAL')}</span> ${t('Earthquakes, storms, fires (may affect data centers)')}</div>
          <div class="layer-help-item"><span>${t('WEATHER')}</span> ${t('Severe weather alerts')}</div>
          <div class="layer-help-item"><span>${t('ECONOMIC')}</span> ${t('Stock exchanges & central banks')}</div>
          <div class="layer-help-item"><span>${t('COUNTRIES')}</span> ${t('Country name overlays')}</div>
        </div>
      </div>
    `;
    ...
    const fullHelpContent = `
      ...
      <div class="layer-help-content">
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Time Filter (top-right)')}</div>
          <div class="layer-help-item"><span>${t('1H/6H/24H')}</span> ${t('Filter time-based data to recent hours')}</div>
          <div class="layer-help-item"><span>${t('7D/30D/ALL')}</span> ${t('Show data from past week, month, or all time')}</div>
          <div class="layer-help-note">${t('Affects: Earthquakes, Weather, Protests, Outages')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Geopolitical')}</div>
          <div class="layer-help-item"><span>${t('CONFLICTS')}</span> ${t('Active war zones (Ukraine, Gaza, etc.) with boundaries')}</div>
          <div class="layer-help-item"><span>${t('HOTSPOTS')}</span> ${t('Tension regions - color-coded by news activity level')}</div>
          <div class="layer-help-item"><span>${t('SANCTIONS')}</span> ${t('Countries under US/EU/UN economic sanctions')}</div>
          <div class="layer-help-item"><span>${t('PROTESTS')}</span> ${t('Civil unrest, demonstrations (time-filtered)')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Military & Strategic')}</div>
          <div class="layer-help-item"><span>${t('BASES')}</span> ${t('US/NATO, China, Russia military installations (150+)')}</div>
          <div class="layer-help-item"><span>${t('NUCLEAR')}</span> ${t('Power plants, enrichment, weapons facilities')}</div>
          <div class="layer-help-item"><span>${t('IRRADIATORS')}</span> ${t('Industrial gamma irradiator facilities')}</div>
          <div class="layer-help-item"><span>${t('MILITARY')}</span> ${t('Live military aircraft and vessel tracking')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Infrastructure')}</div>
          <div class="layer-help-item"><span>${t('CABLES')}</span> ${t('Major undersea fiber optic cables (20 backbone routes)')}</div>
          <div class="layer-help-item"><span>${t('PIPELINES')}</span> ${t('Oil/gas pipelines (Nord Stream, TAPI, etc.)')}</div>
          <div class="layer-help-item"><span>${t('OUTAGES')}</span> ${t('Internet blackouts and service disruptions')}</div>
          <div class="layer-help-item"><span>${t('DATACENTERS')}</span> ${t('AI compute clusters ≥10,000 GPUs')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Transport')}</div>
          <div class="layer-help-item"><span>${t('SHIPPING')}</span> ${t('Vessels, chokepoints, 61 strategic ports')}</div>
          <div class="layer-help-item"><span>${t('DELAYS')}</span> ${t('Airport delays and ground stops (FAA)')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Natural & Economic')}</div>
          <div class="layer-help-item"><span>${t('NATURAL')}</span> ${t('Earthquakes (USGS) + storms, fires, volcanoes, floods (NASA EONET)')}</div>
          <div class="layer-help-item"><span>${t('WEATHER')}</span> ${t('Severe weather alerts')}</div>
          <div class="layer-help-item"><span>${t('ECONOMIC')}</span> ${t('Stock exchanges & central banks')}</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">${t('Labels')}</div>
          <div class="layer-help-item"><span>${t('COUNTRIES')}</span> ${t('Country name overlays')}</div>
          <div class="layer-help-item"><span>${t('WATERWAYS')}</span> ${t('Strategic chokepoint labels')}</div>
        </div>
      </div>
    `;
  ...
  private createLegend(): HTMLElement {
    ...
    if (SITE_VARIANT === 'tech') {
      // Tech variant legend
      legend.innerHTML = `
        <div class="map-legend-item"><span class="legend-dot" style="background:#8b5cf6"></span>${t('Tech HQ').toUpperCase()}</div>
        <div class="map-legend-item"><span class="legend-dot" style="background:#06b6d4"></span>${t('Startup Hub').toUpperCase()}</div>
        <div class="map-legend-item"><span class="legend-dot" style="background:#f59e0b"></span>${t('Cloud Region').toUpperCase()}</div>
        <div class="map-legend-item"><span class="map-legend-icon" style="color:#a855f7">📅</span>${t('Tech Events').toUpperCase()}</div>
        <div class="map-legend-item"><span class="map-legend-icon" style="color:#4ecdc4">💾</span>${t('Datacenter').toUpperCase()}</div>
      `;
    } else {
      // Geopolitical variant legend
      legend.innerHTML = `
        <div class="map-legend-item"><span class="legend-dot high"></span>${t('High Alert').toUpperCase()}</div>
        <div class="map-legend-item"><span class="legend-dot elevated"></span>${t('Elevated').toUpperCase()}</div>
        <div class="map-legend-item"><span class="legend-dot low"></span>${t('Monitoring').toUpperCase()}</div>
        <div class="map-legend-item"><span class="map-legend-icon conflict">⚔</span>${t('Conflicts').toUpperCase()}</div>
